...
void Laser::setVelocity(Velocity velocity)
{
...
bool Laser::hitObject(Lander & lander)
{
	bool result = false;
	float landerX = lander.getPoint().getX();
	float landerY = lander.getPoint().getY();
	float laserX = point.getX();
	float laserY = point.getY();
	float laserProximityX = absoluteValue(landerX - laserX);
	float laserProximityY = absoluteValue(landerY - laserY);
	if ((laserProximityX <= LASER_TOLERANCE) && (laserProximityY <= LASER_TOLERANCE))
	{
		setHasHitObject(true);
		result = true;
	}
	return result;
}

float Laser::absoluteValue(float number)
{
